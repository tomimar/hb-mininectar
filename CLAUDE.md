...
| Sidenav | `hb-sidenav-area` (wrapper) + `hb-sidenav` + `__header`/`__title`/`__toggle`/`__section`/`__group`/`__item`(`--active`/`--expandable`/`--disabled`)/`__icon`/`__label`/`__count`/`__chevron`/`__submenu`/`__subitem`(`--active`). In-page section navigation with optional icons and counts, plus one level of submenu. Collapses away entirely (`is-collapsed`) to give width back to tables, and floats back over the content on hover (`is-peeking`) — the Notion pattern; click the toggle to pin it open. Width via `--hb-sidenav-width` | https://tomimar.github.io/hb-mininectar/components/sidenav.html |
